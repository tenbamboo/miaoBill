...
var Index = {
	...
	showGrayMsg: function(html, tiemout) {
		html = html ? html : '再点击一次退出!';
		tiemout = tiemout ? tiemout : 1500;
		Q.fcall(function() {
			$(".grayMsg").html(html).css({'margin-left': '-' + $(".grayMsg").width() / 2}).show().removeClass("fadeOutUp").addClass("fadeInUp");
			var deferred = Q.defer();
			setTimeout(deferred.resolve, tiemout);
			return deferred.promise;
		}).then(function() {
			$(".grayMsg").removeClass("fadeInUp").addClass("fadeOutUp");
			var deferred = Q.defer();
			setTimeout(deferred.resolve, 500);
			return deferred.promise;
		}).then(function() {
	...
	showRedMsg: function(html, tiemout) {
		...
		tiemout = tiemout ? tiemout : 1500;
		Q.fcall(function() {
			$(".redMsg").html(html).css({'margin-left': '-' + $(".redMsg").width() / 2}).show().removeClass("fadeOutUp").addClass("fadeInUp");
			var deferred = Q.defer();
			setTimeout(deferred.resolve, tiemout);
			return deferred.promise;
		}).then(function() {
	...
	showGreenMsg: function(html, tiemout) {
		...
		tiemout = tiemout ? tiemout : 1500;
		Q.fcall(function() {
			$(".greenMsg").html(html).css({'margin-left': '-' + $(".greenMsg").width() / 2}).show().removeClass("fadeOutUp").addClass("fadeInUp");
			var deferred = Q.defer();
			setTimeout(deferred.resolve, tiemout);
			return deferred.promise;
		}).then(function() {
